...
export async function buildRecipe(suggestion, ageLabel, useSonnet = false) {
  const modelTag = useSonnet ? "sonnet" : "haiku";
  const cacheKey = `na-recipe-${new Date().toLocaleDateString("es-ES").replace(/\//g, "-")}-${modelTag}`;
